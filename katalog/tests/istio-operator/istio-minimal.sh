...
@test "deploy bookinfo demo application" {
  ...
  wait_for_it() {
    ...
    retry_counter=0
    max_retry=10
    while curl -s localhost:31370/productpage |grep "Error fetching product reviews!"  >&2
    do
      [ $retry_counter -lt $max_retry ] || ( kubectl describe all -n demo >&2 && return 1 )
      sleep 2 && echo "# waiting..." $retry_counter >&3
      retry_counter=$(( retry_counter + 1 ))
...
@test "test v1 bookinfo demo application" {
  info
  test_back_stars(){
    curl -s localhost:31370/productpage |grep -q 'color="black"'
  }
  run test_back_stars
  back_stars_result=$status
  test_red_stars(){
    curl -s localhost:31370/productpage |grep -q 'color="red"'
  }
  run test_red_stars
...
@test "test v2 for jason in bookinfo demo application" {
  ...
  test(){
    ...
    do
        ...
        if [ $retry_counter -ge $max_retry ]; then echo -n "Timeout waiting a condition" >&3; curl -s localhost:31370/productpage >&2; return 1; fi
        rm -rf ${BATS_TMPDIR}/test-cookie-${CLUSTER_NAME}.txt
        curl -s -L -c ${BATS_TMPDIR}/test-cookie-${CLUSTER_NAME}.txt -d "username=jason" -d "passwd=jason" http://localhost:31370/login
        output=$(curl -s -b ${BATS_TMPDIR}/test-cookie-${CLUSTER_NAME}.txt localhost:31370/productpage )
        curl -s -b ${BATS_TMPDIR}/test-cookie-${CLUSTER_NAME}.txt localhost:31370/productpage |grep -q 'color="black"'
        ko=$?
...
@test "test v3 for all but jason in bookinfo demo application" {
  ...
  test(){
    ...
    while [[ ko -eq 1 ]]
    do
        if [ $retry_counter -ge $max_retry ]; then echo "Timeout waiting a condition"; curl -s localhost:31370/productpage >&2; return 1; fi
        curl -s localhost:31370/productpage |grep -q 'color="red"'
        ko=$?
